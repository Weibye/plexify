...
mod tests {
    ...
    async fn test_clean_empty_directory() {
        ...
        let clean_cmd = CleanCommand::new(temp_dir.path().to_path_buf(), temp_dir.path().to_path_buf());
    ...
    async fn test_clean_nonexistent_directory() {
        let clean_cmd = CleanCommand::new(PathBuf::from("/nonexistent/path"), PathBuf::from("/tmp"));
