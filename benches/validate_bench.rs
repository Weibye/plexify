...
fn create_test_media_library(size: usize) -> TempDir {
    ...
    for i in 0..size {
        ...
        if i % 10 < 7 {
            fs::write(
                show_path.join(format!("Test Show {} - s01e{:02} - Episode {}.mkv", i % 10, (i % 24) + 1, i)),
                "",
            ).unwrap();
        } else {
            // Create incorrectly named files (30%)
            fs::write(
                show_path.join(format!("episode_{}.mkv", i)),
                "",
            ).unwrap();
        }
        ...
        // Add some movies too
        if i % 20 == 0 {
            let movie_path = media_root.join(format!("Movies/Test Movie {} ({})", i / 20, 2000 + (i / 20)));
